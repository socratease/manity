...
import { exportPortfolio, importPortfolio, loadPortfolio, savePortfolio } from './lib/data/portfolioPersistence';
...
export default function ManityApp({ onOpenSettings = () => {} }) {
  ...
  const [portfolioMessageTone, setPortfolioMessageTone] = useState('success');
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => setPortfolioMessage(null), 4000);
  ...
  const showPortfolioFeedback = (message, tone = 'success') => {
    setPortfolioMessage(message);
    setPortfolioMessageTone(tone);
  };

  const handleExport = () => {
    try {
      const json = exportPortfolio(projects);
      ...
      showPortfolioFeedback('Portfolio exported as JSON.');
    } catch (error) {
      showPortfolioFeedback('Unable to export portfolio.', 'error');
    }
  };

  const handleImport = async (source) => {
    try {
      const imported = await importPortfolio(source);
      setProjects(imported);
      setViewingProjectId(null);
      setActiveView('overview');
      setExpandedTasks({});
      setSelectedProject(null);
      showPortfolioFeedback('Portfolio imported successfully.');
    } catch (error) {
      showPortfolioFeedback(error.message || 'Unable to import portfolio.', 'error');
    }
  };
    ...
    <>
      ...
      <main style={styles.main}>
        <div style={styles.topBar}>
          <div style={styles.portfolioMessageArea}>
            {portfolioMessage && (
              <div style={portfolioMessageTone === 'error' ? styles.portfolioMessageError : styles.portfolioMessageSuccess}>
                {portfolioMessage}
              </div>
            )}
          </div>
          <div style={styles.topBarRight}>
            <div style={styles.portfolioActions}>
              <button onClick={handleExport} style={styles.tertiaryButton}>
                Export
              </button>
              <button onClick={triggerImportDialog} style={styles.tertiaryButton}>
                Import
              </button>
              <input
                ref={importInputRef}
...
const styles = {
  ...
  portfolioMessageError: {
    padding: '10px 14px',
    borderRadius: '12px',
    backgroundColor: 'rgba(214, 124, 92, 0.12)',
    border: '1px solid rgba(214, 124, 92, 0.5)',
    color: 'var(--charcoal)',
    fontFamily: "'Inter', sans-serif",
    fontSize: '13px',
  },
