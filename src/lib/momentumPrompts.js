...
- update_project: change project fields. Fields: projectId, name (rename project), description (project description), executiveUpdate (executive summary), status (planning/active/on-hold/cancelled/completed), priority (low/medium/high), progress (0-100), targetDate, startDate, lastUpdate. Use project statuses: planning, active, on-hold, cancelled, or completed (never "in_progress"). Only include update_project.description when the user explicitly asks to change the description; progress/status updates must never modify descriptions unless requested. Example: user says "Update Project A's status to active" -> update_project with status only (no description).
